...
#ifdef DEBUG
  NSLog(@"decoding: image to decode is (%zd x %zd) (%zd bytes/row)", subsetWidth, subsetHeight, subsetBytesPerRow);
#endif
...
#ifdef DEBUG
  NSLog(@"created %zdx%zd bitmap context", subsetWidth, subsetHeight);
#endif
...
#ifdef DEBUG
  NSLog(@"drew image into %zd(%zd)x%zd  bitmap context", subsetWidth, subsetBytesPerRow, subsetHeight);
#endif
  CGContextFlush(ctx);
#ifdef DEBUG
...
- (BOOL)decode {
  ...
  { 
    ...
    binarizer = 0;
#ifdef DEBUG
    NSLog(@"created GreyscaleLuminanceSource(%p,%zd,%zd,%d,%d,%zd,%zd)",
